...
test("should render all menu items", () => {
    const { container } = render(
        <SideBar MenuItems={[{ Id: 1, Name: "Home" }]} position={"left"} Responsive={false} />
    );
...
test("drawer should be by default left position", () => {
    const { container } = render(
        <SideBar MenuItems={[]} position={"left"} Responsive={false} />
    );
...
test("should render divider ", () => {
    const { container } = render(
        <SideBar MenuItems={[]} Responsive={false} />
    );
