...
export function TableComponent(props: TableProps) {
  ...
  useEffect(() => {
    const filterHeading = tableHeader.filter(el =>
  ...
  const filterHead =
    filteredTableHeading.length > 0 ? filteredTableHeading : tableHeader;
                    ...
                    .map((row, index) =>
                      stripe ? (
                        ...
                        <TableRow hover={hover} key={index}>
                          {filterHead.map((el, index) => (
                            <TableCell key={index}>{row[el.id]}</TableCell>
                          ))}
                        </TableRow>
                      ),
                    )
