...
import {
  Button, ClickAwayListener, Grow, IconButton, Menu, TextField, Tooltip, Typography,
} from '@mui/material';
...
export function ToolbarComponent(props:ToolbarProps) {
  const {
    title, search, tableHeader, filterHead, newData, exportData, searchText, tableDataSearch, setSearchText, selectColumn, checkedValues, columnFilter, pagination,
  } = props;
  ...
  const defaultExportCsv = () => {
    ...
    const data:any = newData.map((rowData:any) => columns.map((columnDef) => rowData[columnDef.id]));
  ...
  return (
    <Toolbar
      ...
      style={{
        display: 'flex', flexDirection: 'row', justifyContent: 'space-between', padding: 0,
      }}
    >
      <div>
        {showSearch
              ...
              <ClickAwayListener onClickAway={() => setIsOpen(false)}>
                <div className="popOverMenu">
                  {tableHeader.map((el, index) => (
                    <div key={index} className="tablePopupContainer">
                      <Button
